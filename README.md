...
Локальный генератор карточек товара (заголовок, краткое описание и пункты) через Ollama. Выход — строгий JSON. Есть минимальный CLI и Telegram‑бот‑скелет.
...
Сценарий: `/start` → выбрать платформу → одним сообщением отправить «название + характеристики» → получить сгенерированный текст. Хранилища и экспорт файлов нет (войдут в 0.3).

## Документация
- Дорожная карта версий: `docs/ROADMAP.md` (текущий статус: v0.2)
